...
# ---------------- CLASS NAMES ----------------
# 🔴 UPDATE if your class order is different
CLASS_NAMES = ["Red Soil", "Black Soil", "Alluvial Soil", "Clay Soil"]
...
# ---------------- POSTPROCESS ----------------
def postprocess(preds, img_shape, conf_thres=0.4):
    h, w = img_shape
    detections = []

    preds = preds[0]  # (num_boxes, 85)
    ...
    for pred in preds:
        ...
        if obj_conf > conf_thres:
            ...
            detections.append((x1, y1, x2, y2, class_id, class_conf))
...
if uploaded_file:
    ...
    st.info("👆 Upload a soil image to start detection.")
